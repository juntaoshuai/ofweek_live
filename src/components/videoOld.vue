<template>
    <nav class="myvedio" v-bind:style="{backgroundImage:'url('+reviewUrl+')'}">
        ...
        <div class="vedio" v-if="model==0">
            <video v-if="hlsdownstream" id="hlsVideo" v-bind:src=hlsdownstream v-bind:poster="hlsimg" webkit-playsinline playsinline
 controls autoplay></video>
        ...
        <div class="vodlook" v-if="model==8">
            <video v-bind:src="vodliving" v-bind:poster="reviewUrl" webkit-playsinline playsinline controls id="vodliving"></video>
        </div>

        <!-- 查看回顾 -->
        <div class="vodlook" v-if="model==6">
            <video v-bind:src="vodvideo" v-bind:poster="reviewUrl" webkit-playsinline playsinline controls id="myAudio" onended="myFunction()"></video>
        </div>
    </nav>
</template>
